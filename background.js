// Social Media Downloader Background Service Worker - Direct Native Stream Version
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'download' && request.url) {
    chrome.storage.local.get(['filenameFormat', 'customPrefix', 'downloadHistory'], (data) => {
      ...
      // Use Chrome native downloader directly to ensure clean MP4 binary stream without corruption
      chrome.downloads.download(
        ...
        (downloadId) => {
          if (chrome.runtime.lastError) {
            ...
            console.log('[Social Media Downloader] Download started successfully:', filename, downloadId);
